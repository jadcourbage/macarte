...
function popupRates(txreu, txmention, examLabel, year, ips, ipsRentree) {
    if (txreu == null) {
    ...
    const ipsLine = ips != null
        ? '<p style="margin:4px 0 0;font-size:11px"><span style="color:#999">IPS\u00a0(' + escapeHtml(String(ipsRentree || '')) + ')\u00a0:</span> <b>' + Math.round(ips) + '</b></p>'
        : '';
    return '<hr style="margin:6px 0">' +
        ...
        '</div>' +
        '<p style="margin:4px 0 0;color:#999;font-size:11px">R\u00e9sultats ' + examLabel + yearStr + '</p>' +
        ipsLine;
}
...
function infoCol(nom, adresse, codepostal, type, txreu, txmention, brevetSession, ips, ipsRentree) {
    return popupSchoolHeader(nom, type) +
        '<p style="margin:0 0 4px;color:#555">' + escapeHtml(adresse) + '<br>' + escapeHtml(codepostal) + ' PARIS</p>' +
        popupRates(txreu, txmention, 'brevet', brevetSession, ips, ipsRentree);
}

// Generate HTML content for college popup with walking distance
function infoColPath(nom, adresse, codepostal, type, txreu, txmention, time, distance, brevetSession, ips, ipsRentree) {
...
function infoLyc(nom, adresse, codepostal, type, txreu, txmention, bacAnnee) {
    return popupSchoolHeader(nom, type) +
        '<p style="margin:0;color:#555">' + escapeHtml(adresse) + '<br>' + escapeHtml(codepostal) + ' PARIS</p>' +
        popupRates(txreu, txmention, 'bac', bacAnnee);
}
