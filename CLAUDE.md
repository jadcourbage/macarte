...
- **data/*.geojson**: Geographic data files for schools and sectors
  - `colleges.geojson`: College locations (Point) + sector polygons (MultiPolygon) in a single file
...
- Sectors displayed as fill/line layers with `map.setPaintProperty()` for style switching
